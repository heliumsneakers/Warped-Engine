...
//  Tunables
// --------------------------------------------------------------------------
static constexpr float DEFAULT_LUXEL_SIZE = 2.0f;   // world units per luxel
...
static OccluderSet BuildOccluders(const std::vector<MapPolygon>& polys) {
    ...
    if (hiddenCount > 0) {
        printf("[Lightmap] skipped %d hidden coplanar brush faces from occluders.\n", hiddenCount);
    }
    return o;
}
...
static bool HasInvalidDarkValidLuxels(const LightmapPage& page,
                                      const std::vector<uint8_t>& valid)
{
    const size_t pixelCount = (size_t)page.width * (size_t)page.height;
    if (valid.size() < pixelCount || page.pixels.size() < pixelCount * 4) {
        return true;
    }
                ...
                for (int sy = 0; sy < AA_GRID; ++sy) {
                    for (int sx = 0; sx < AA_GRID; ++sx) {
                        ...
                        for (uint32_t lightIndex : r.lightIndices) {
                            ...
                            if (ndl <= 0) continue;
                            if (Occluded(occ, ro, dir, std::max(0.0f, dist - (SHADOW_BIAS * 2.0f)), L.ignoreOccluderGroup)) continue;
                           ...
                           const std::vector<PointLight>& lights)
{
    ...
    for (uint32_t pageIndex = 0; pageIndex < atlas.pages.size(); ++pageIndex) {
        ...
        if (!usedCPUFallback && HasInvalidDarkValidLuxels(page, valid)) {
            printf("[Lightmap] page %u compute bake produced invalid dark valid texels, falling back to CPU.\n",
                   pageIndex);
            fflush(stdout);
            BakeLightmapCPUPage(patches, lights, rects, pageIndex, occ, page);
